...
RSpec.shared_examples "AutoDetectBackend" do |selected_db|
  describe 'FilterModel backend auto-detection' do
    context 'when only one adapter is in use (this context\'s DB)' do
      let!(:ada) { Author.create!(first_name: 'Ada', last_name: 'Lovelace') }
      let!(:article_1) { Article.create!(title: 'The First Article', content: 'Alpha', published: DateTime.now, author: ada) }
      let!(:article_2) { Article.create!(title: 'Second', content: 'Beta', published: DateTime.now, author: ada) }
      ...
      it 'uses the DSL without specifying db: (auto-detected from the model connection)' do
        klass = Class.new do
          ...
          filter_model :article # no db: specified; should auto-detect from Article connection
          define_query_key :q
